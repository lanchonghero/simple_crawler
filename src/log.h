...
({\
    ...
    char __strftime_buf__[sizeof("19861113 23:59:59")];\
    time(&__time_buf__);\
    localtime_r(&__time_buf__, &__localtime_buf__);\
    strftime(__strftime_buf__, sizeof("19861113 23:59:59"), "%Y%m%d %H:%M:%S", &__localtime_buf__);\
...
({\
    ...
    localtime_r(&__time_buf__, &__localtime_buf__);\
    strftime(__strftime_buf__, sizeof("19861113 23:59:59"), "%Y%m%d %H:%M:%S", &__localtime_buf__);\
    fprintf(stderr, "[WARNING]  [%s] [%08lX] " CODE_INFO_FORMAT fmt "\n",\
            __strftime_buf__, pthread_self(), CODE_INFO, ##__VA_ARGS__);\
})
...
({\
    ...
    static_cast<int>((__now__.tv_sec - past.tv_sec) * 1000000 + (__now__.tv_usec - past.tv_usec));\
})

#define WASTE_TIME_US_INTERVAL(start, end)\
({\
    static_cast<int>((end.tv_sec - start.tv_sec) * 1000000 + (end.tv_usec - start.tv_usec));\
})
...
({\
    timeval __now__;\
    gettimeofday(&__now__, NULL);\
    static_cast<int>((__now__.tv_sec - past.tv_sec) * 1000 + (__now__.tv_usec - past.tv_usec) / 1000);\
})

#define WASTE_TIME_MS_INTERVAL(start, end)\
({\
    static_cast<int>((end.tv_sec - start.tv_sec) * 1000 + (end.tv_usec - start.tv_usec) / 1000);\
})
